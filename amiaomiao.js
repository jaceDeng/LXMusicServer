...
const apis = {
  kw: {
    musicUrl(info, quality) {
      return httpRequest('https://XXX/musicapi?q='+quality,{
        method: 'POST', // 设置请求方法为 POST
        body: JSON.stringify(info), // 将数据转换成 JSON 字符串
        headers: { // 设置请求头，告诉服务器我们发送的是 JSON 数据
  ...
  // 支持的源对象，可用key值：kw/kg/tx/wy/mg/local
  kg: {
    musicUrl(info, quality) {
      return httpRequest('https://XXX/musicapi?q='+quality,{
        method: 'POST', // 设置请求方法为 POST
  ...
  local: {
    musicUrl(info) {
      return httpRequest('https://XXX').then(data => {
        return data.url
      })
    },
    pic(info) {
...
send(EVENT_NAMES.inited, {
  ...
  sources: { // 当前脚本支持的源
    ...
    kg: { // 支持的源对象，可用key值：kw/kg/tx/wy/mg/local
      name: '酷狗',
      type: 'music',  // 目前固定为 music
      actions: ['musicUrl'], // 除了local外，其他的固定为 ['musicUrl']
      qualitys: ['128k', '320k', 'flac', 'flac24bit'], // 当前脚本的该源所支持获取的Url音质，有效的值有：['128k', '320k', 'flac', 'flac24bit']
    },
    tx: { // 支持的源对象，可用key值：kw/kg/tx/wy/mg/local
      name: 'QQ音乐',
      type: 'music',  // 目前固定为 music
      actions: ['musicUrl'], // 除了local外，其他的固定为 ['musicUrl']
      qualitys: ['128k', '320k', 'flac', 'flac24bit'], // 当前脚本的该源所支持获取的Url音质，有效的值有：['128k', '320k', 'flac', 'flac24bit']
